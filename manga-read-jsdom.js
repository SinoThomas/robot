...
(async () => {
  ...
  for (let i = 0; thereIsNextChapter; i++) {
    ...
    if (chapterExist && !overwrite) {
      logOnSameLine(`Chapter ${chapter.padStart(5)}     Skipped    in ${getDuration(true)}ms`);
    } else {
      ...
      logOnSameLine(`Chapter ${chapter.padStart(5)}     Added ${String(imagesEls?.length).padStart(3)} images    in ${getDuration(true)}ms`);
    }
